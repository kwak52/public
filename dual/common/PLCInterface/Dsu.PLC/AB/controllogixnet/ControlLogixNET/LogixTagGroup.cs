...
namespace ControlLogixNET
{
    ...
    /// </summary>
    internal sealed class LogixTagGroup
    {
        ...
        {
            SessionInfo si = _parent.SessionInfo;
            string tagAddress = tag.Address;
            ...
            int temp = 0;
            ReadDataServiceRequest request = LogixServices.BuildLogixReadDataRequest(tag.Address, tag.Elements, out temp);

            //Now we read it from the PLC to find out if it's fragmented...
            CommonPacketItem addressItem = CommonPacketItem.GetConnectedAddressItem(si.ConnectionParameters.O2T_CID);
            CommonPacketItem dataItem = CommonPacketItem.GetConnectedDataItem(request.Pack(), SequenceNumberGenerator.SequenceNumber);

            EncapsReply reply = si.SendUnitData(addressItem, dataItem);
            ...
            if (reply != null)
            {
                ...
                if (rdReply.Status == 0x06)
                {
                    ...
                    request = LogixServices.BuildFragmentedReadDataRequest(tag.Address, tag.Elements, 0, out temp);
