...
#pragma warning restore 169
    ...
    internal class resultN
    {
        public int error;
        public byte[] bytes;
    }
    internal class daveResultN :IresultSet
    {
    ...
    internal class TcpNETdave : IDaveConnection, IDisposable
    {
        ...
        */
        ...
        public int execReadRequest(IPDU p, IresultSet rl)
        {
            ...
            //res=_daveExchange(dc, p);
            Pdu p2 = ExchangePdu((Pdu)p);
            //if (res!=daveResOK) return res;
            //res=_daveSetupReceivedPDU(dc, &p2);
            //if (res!=daveResOK) return res;
            int res = p2.testReadResultMulti();
            if (res != Connection.daveResOK) return res;
            int i = 0;
            if (rl != null)
            {
                ...
                //rl->results=cr;
                //c2=cr;
                byte[] q = p2.Data.ToArray();
                //byte[] qU = p2.UData.ToArray();
                int countQ = 0;

                int rlen = p2.Data.Count;
                int len;
                while (i < p2.Param[1])
                {
                    cr = new resultN();
