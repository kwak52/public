...
namespace NationalInstruments.Examples.ContAcqVoltageSamples_IntClk
{
    ...
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        private const int NumberOfSamplePerChannel = 100;


        private CircularBuffer<DAQData> _buffer = new CircularBuffer<DAQData>(9000); // 1 sec
        ...
        private void startButton_Click(object sender, System.EventArgs e)
        {
            if (runningTask == null)
            {
                try
                {
                    ...
                    myTask.AIChannels.CreateVoltageChannel("Dev5/ai0", "",
                    ...
                    myTask.AIChannels.CreateVoltageChannel("Dev5/ai1", "",
                        (AITerminalConfiguration)(-1), Convert.ToDouble(minimumValueNumeric.Value),
                        Convert.ToDouble(maximumValueNumeric.Value), AIVoltageUnits.Volts);

                    myTask.AIChannels.CreateVoltageChannel("Dev5/ai2", "",
                        (AITerminalConfiguration)(-1), Convert.ToDouble(minimumValueNumeric.Value),
                        Convert.ToDouble(maximumValueNumeric.Value), AIVoltageUnits.Volts);

                    myTask.AIChannels.CreateVoltageChannel("Dev5/ai3", "",
                        (AITerminalConfiguration)(-1), Convert.ToDouble(minimumValueNumeric.Value),
                        Convert.ToDouble(maximumValueNumeric.Value), AIVoltageUnits.Volts);

                    // Configure the timing parameters
                    myTask.Timing.ConfigureSampleClock("", 100000/*Convert.ToDouble(rateNumeric.Value)*/,
                        SampleClockActiveEdge.Rising,
                        SampleQuantityMode.ContinuousSamples,
                        NumberOfSamplePerChannel);
                    ...
                    analogInReader.BeginReadWaveform(NumberOfSamplePerChannel/*Convert.ToInt32(samplesPerChannelNumeric.Value)*/,
        ...
        {
            Console.WriteLine($"Reading {_counter}-th.");
            if (_counter++ == 1000)
            ...
            try
            {
                if (runningTask != null && runningTask == ar.AsyncState)
                {
                    ...
                    var raw0 = data[0].GetRawData();
                    var raw1 = data[1].GetRawData();
                    var raw2 = data[2].GetRawData();
                    var raw3 = data[3].GetRawData();

                    var now = DateTime.Now;
                    for ( int r = 0; r < NumberOfSamplePerChannel; r++ )
                    {
                        var data2 = new DAQData(now, raw0[r], raw1[r], raw2[r], raw3[r]);
                        _buffer.Enqueue(data2);
                    }
