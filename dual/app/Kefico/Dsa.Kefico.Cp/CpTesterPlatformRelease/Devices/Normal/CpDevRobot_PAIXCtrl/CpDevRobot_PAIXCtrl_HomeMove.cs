...
{
#if false
    partial class CpDevRobot_PAIXCtrl
    {
        private async Task AwaitMotionFinish(CancellationToken cancelToken, params short[] axes)
        {
            ...
            var succeeded =
                Prelude.match(fs(_paix.GetStopInfo()),
                    ...
                    None: () => false);
            if (!succeeded)
                throw new ExceptionWithCode(ErrorCodes.USR_OperationCancel, "Waiting motion finish failed by user operatino cancel request.");
        }
        ...
        private async Task<bool> AwaitHomeMoveFinish(CancellationToken cancelToken, params short[] axes)
        {
                ...
                while (true)
                {
                    var finished =
                        Prelude.match(fs(_paix.GetHomeStatus()),
                            Some: v => axes.ForAll(ax => v.nStatusFlag[ax] != 1),
                            None: () => false);
                    ...
                    if (finished)
                    {
                        ...
                        var succeeded =
                            Prelude.match(fs(_paix.GetStopInfo()),
                                ...
                                None: () => false);
                        if (!succeeded)
                            throw new ExceptionWithCode(ErrorCodes.DEV_PaixError, "Failed to move home.");
                                    ...
                                    None: () => false))
                        {
                            ...
                            if (n++ > 200)     // 20 seconds max
                                throw new ExceptionWithCode(ErrorCodes.DEV_PaixError, "Failed to move home : InPosition not confirmed.");
                        }
            ...
            {
                LogInfoRobot("finished home move finish.");
            }
        }
        ...
        private void HomeMovePrologue(IEnumerable<short> axes)
        {
            ...
            var errorMessage = CheckErrorStatus(realAxes);
            if (errorMessage.Any())
                throw new ExceptionWithCode(ErrorCodes.DEV_PaixError, $"ERROR on robot: {errorMessage.Aggregate((agg, next) => next + agg)}");
        ...
        public Task<bool> HomeMove(CancellationToken cancelToken)
        {
            if (DriverBaseGlobals.IsAudit78())
                return HomeMoveAudit78(cancelToken);
            else
                return HomeMoveAuditGCVT(cancelToken);
        }
        ...
        /// <returns></returns>
        private async Task<bool> HomeMoveAuditGCVT(CancellationToken cancelToken)
        {
            ...
            try
            {
               ...
               _paix.SetHomeSpeedAccDec(theAxis,
                ...
                if (!succeeded)
                {
                    ...
                    throw new ExceptionWithCode(ErrorCodes.DEV_PaixError, "Home set error");
                }
                ...
                if (!stopStatusOK)
                    throw new ExceptionWithCode(ErrorCodes.DEV_PaixError, "Not Encorder Z stopped.");
        ...
        /// 4. 
        /// </summary>
        private async Task<bool> HomeMoveAudit78(CancellationToken cancelToken)
        {
